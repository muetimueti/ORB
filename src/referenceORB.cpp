...
referenceORB::referenceORB(int _nfeatures, float _scaleFactor, int _nlevels,
...
void referenceORB::ComputeKeyPointsOld(std::vector<std::vector<KeyPoint> > &allKeypoints)
{
    allKeypoints.resize(nlevels);
...
void referenceORB::operator()( InputArray _image, InputArray _mask, vector<KeyPoint>& _keypoints,
...
void referenceORB::ComputePyramid(cv::Mat image)
{
    for (int level = 0; level < nlevels; ++level)
    {
